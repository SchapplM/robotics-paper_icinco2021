...
An efficient scheme for difference-quotient approximation of gradients of performance criteria for projection into the nullspace of the task redundancy is presented.
The simulation example of a hexapod robot shows that avoiding and exiting parallel robots singularities of type II is possible with the nullspace of all joints.
The nullspace controller scheme can be used in offline trajectory optimization and in online motion generation.
}
% manipulators in pointing tasks, redundancy allows nullspace optimization, special case of degree of redundancy of one, definition of orientation coordinates with Cardan angles allows an efficient and unified formulation for serial and parallel robots, present efficient projection scheme of gradients of performance criteria for projection into the nullspace of the task redundancy, this allows to avoid singularities and especially for parallel robots to leave singularities of type II
...
% no citations in the following sentence (if at all, should be basic reference, not 3 year old paper)
Further, the IKP can be distinguished between position level and velocity level or higher differential order. % \cite{SchapplerTapOrt2019}  , see \cite{ReiterMueGat2018}
...
\item orthogonal decomposition of the task space \cite{LegerAng2016,CorinaldiAngCal2016},% (for serial robots) and \cite{CorinaldiAngCal2016} for two collaborating parallel robots
%\item removing the row of the Jacobian corresponding to redundant coordinate \cite{Zlajpah2017}, 
\item reciprocal Euler angles %
%\footnote{The term ``Euler angles'' is used for generalization and is not limited to ``proper Euler angles''. However, using Cardan angles (known as Tait-Bryan angles) is favorable.}
...
Examples are end milling \cite{ShawChe2001} or milling with a spherical cutter \cite{SmirnovPlyMir2013}. % \cite{ShawChe2001} (using a genetic algorithm for optimization)
...
%The former are well-established, see \cite{NakamuraHanYos1987} and \cite{ChiaveriniOriWal2008} and allow efficient computation.
...
%Sample references:
%\begin{compactitem}
%\item Reviews for parallel robot redundancy: \cite{LucesMilBen2017}, \cite{GosselinSch2018}
%\item Methods for optimization of parallel kinematic machine tools: \cite{ShawChe2001} (end milling with three redundant orientations), \cite{SmirnovPlyMir2013}
%\item parallel robot task redundancy: \cite{GaoCheGaoXia2019} (small iterations of redundant coordinate), \cite{MerletPerDan2000} (interval analysis), \cite{OenWan2007} (linear programming, iterative procedure), \cite{AgarwalNasBan2016}, \cite{SchapplerTapOrt2019} (minimal coordinate formulation on position level), \cite{CorinaldiAngCal2016} (two cooperating parallel robots)
%\item kinematically redundant parallel robots: \cite{KotlarskiDoHeiOrt2010}, \cite{GosselinSch2016}, \cite{SantosSil2017}
%\item task redundancy of serial robots: \cite{ReiterMueGat2018} (important remark on consistency of differential order), \cite{Zlajpah2017} (removing row corresponding to redundant task coordinate)
%\item Further examples for task redundancy with serial robots: \cite{GuoDonKe2015} (incrementing redundant coordinate for global optimization), \cite{MousaviGagBouRay2018} (optimization milling trajectory), \cite{LegerAng2016} (orthogonal decomposition of the task space), \cite{HuoBar2008} (twist decomposition)
%\item foundations of redundancy resolution: \cite{NakamuraHanYos1987} (nullspace projection), \cite{DeLucaOriSic1992} (PD controller for nullspace term)
%different methods for robot redundancy resolution: \cite{Ozgoren2013} (analytic expression of redundant coordinates)
%\item kinematics model of PKM \cite{Merlet2006a}
%\end{compactitem}
...
%This order does not facilitate the task redundancy.
...
%Methods for compensation could be switching to a different convention in that case.

%task coordinates 
The \emph{task space coordinates} are defined as ${\bm{y}=(r_x,r_y,r_z,\varphi_x,\varphi_y)^\transp}$.
Due to the rotational symmetry of the robot tool and the chosen representation of rotation, the last operational space coordinate $\varphi_z$ corresponds to a rotation of the robot end effector around the tool axis and can be selected arbitrarily in the regarded case of task redundancy. %, which produces the task redundancy and can be exploited for optimization.
...
\begin{equation}
\bm{q}^{k+1}=\bm{q}^{k} - 
%\Delta \bm{q}^{k} = \Delta \bm{q}^{k}_\mathrm{T} + \Delta \bm{q}^{k}_\mathrm{N} =
...
% Case I: General Case, especially 
...
%Position-Level IKP, $\lVert\bm{\Psi}\rVert \not\approx 0$
%% Optimization function $h=\mathrm{cond}(\bm{J}(\bm{q}))$
%
%Gradient in the general case  and $\Delta \bm{q} = (0,\hdots 0, \Delta q_i, 0 \hdots 0)$
%
...
%Gradient of the nested function $\bm{q}(\bm{x})$ gives $h_{\partial \bm{q}} = (\partial h / \partial \bm{x}) (\partial \bm{x} / \partial \bm{q})$

%Case II: General Case, especially $\lVert\bm{\Psi}\rVert \approx 0$

%Gradient in the case of one-DoF task redundancy $h_{\partial \bm{q}} = (\partial h / \partial \varphi_z) (\partial \varphi_z / \partial \bm{q})$

%Approximation is valid because other operational space motion is removed by the nullspace projection. No freedom to move in other directions.
...
%this effect is overruled by the nullspace motion.
%
...
%Therefore it is applicable to any kind of parallel robot (without)

%if a minimal kinematics set exists, $\bm{\Phi}(\bm{\theta},\bm{x})$ is defined. 

%The reduced constraints $\bm{\Psi}(\bm{\theta},\bm{y})$ for task redundancy can not be defined in the minimal kinematics set.
...
%Case I: General Case, especially $\lVert\bm{\Psi}\rVert \not\approx 0$, see serial robots. For parallel robots with joint space objectives $h(\bm{q})$ identical.
...
%Optimization function expressed in full coordinates $h=\mathrm{cond}(\bm{J}(\bm{q},\bm{x}))$

%Nullspace motion in actuation space

%numeric approximation (no analytic expression available)
\begin{equation}
...
%One Nullspace component in the acceleration level is not equivalent to the derivative of the velocity-level solution \cite{ReiterMueGat2018}.
%To avoid the analytic differentiation, the scheme from \cite{DeLucaOriSic1992} can be used, Fig.~\ref{fig:control_loop}.
%
%The scheme in \cite{AgarwalNasBan2016} is similar, but with $K_\mathrm{v}=0$ and $K_\mathrm{D}=0$.
...
%TODO: Eventually remove this sentence for layout reasons (but it comes later again anyway)
%The nullspace motion controller can be used either in offline planning or in online control of the robot in the case of the robot leaving the reference trajectory, \cite{AgarwalNasBan2016}.

%In the linear case, this leads to undamped oscillations, but can be stabilized with the PD controller structure.
%
%Implementation using discrete time variables
...
%The first starting pose is set to a non-singular value at $\varphi_{z,\mathrm{start},1}{=}$\SI{0}{\degree}.
...
Therefore, the nullspace motion is obtained using the \emph{trajectory IK scheme} from (\ref{eq:par_nullspace_actjoint}) and Sec.~\ref{sec:parrob_taskred_actspace} in the \emph{actuator space}. % (\ref{eq:par_nullspace_actjoint})
...
%
%TODO: Simulation case study of trajectory tracking of the parallel robot with optimization of condition number at the same time.


\begin{table}[b] % see Matlab script
	...
	\begin{tabular}[t]{|c|c|c|c|}
		...
		stroke limits & $\theta_{i,\mathrm{min/max}}$ & 600, 1200 & mm\\ % \SI{600}{\milli\metre}, \SI{1200}{\milli\metre} % ,$\theta_{i,\mathrm{max}}$
...
%While many publications cover the fundamental theory of redundancy resolution, the proposed extension enables a faster computation of the p
